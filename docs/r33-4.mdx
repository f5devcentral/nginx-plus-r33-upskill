...
1. From UDF, open up a Web Shell for your NGINX Plus instance (UDF Component NGINX Plus R33 (Preinstall)).

1. Install NGINX Plus by running `sudo apt update` followed by `sudo apt install nginx-plus`

1. Create your JWT file at `/etc/nginx/license.jwt` using the JWT file you used to license the NGINX Instance Manager in the previous steps. 
...
command in the form of another UUID at the end of the "statusLink" value.
...
`acknowledgement.zip`. Remember to swap in the UUID for \<report_id\>.
